/* 
 * $Id: DXVADecoderH264.cpp 1308 2009-10-25 13:21:21Z casimir666 $
 *
...
#define CHECK_HR(x)			hr = ##x; if (FAILED (hr)) { CLog::Log(LOGERROR, "CDXVADecoderH264 Error : 0x%08x\n", hr); return hr; }

CDXVADecoderH264::CDXVADecoderH264 (CDVDVideoCodecDXVA* pFilter, IDirectXVideoDecoder* pDirectXVideoDec, DXVAMode nMode, int nPicEntryNumber, DXVA2_ConfigPictureDecode* pDXVA2Config)
...
HRESULT CDXVADecoderH264::DisplayStatus()
{
  ...
  CLog::Log(101, "CDXVADecoderH264 : Status for the frame %u : bBufType = %u, bStatus = %u, wNumMbsAffected = %u Index7Bits = %u\n", 
		Status.StatusReportFeedbackNumber,
		Status.bBufType,
		Status.bStatus,
		Status.wNumMbsAffected,
    Status.CurrPic.Index7Bits);
...
int CDXVADecoderH264::PrintRefImages()
{
  int counter = 0;

  for(int i = 0; i < 16; i++)
  {
    CLog::Log(LOGDEBUG, "PicParams %d: AssociatedFlag = %d Index7Bits = %d\n", 
      i, m_DXVAPicParams.RefFrameList[i].AssociatedFlag, m_DXVAPicParams.RefFrameList[i].Index7Bits);
    if(m_DXVAPicParams.RefFrameList[i].Index7Bits != 127) counter++;
  }
  return counter;
}

int CDXVADecoderH264::PrintPictureStore()
{
  int counter = 0;

  for(int i = 0; i < m_nPicEntryNumber; i++)
  {
    CLog::Log(LOGDEBUG, "PictureStore %d: RefPicture = %d Displayed = %d inUse = %d\n",
    i, m_pPictureStore[i].bRefPicture, m_pPictureStore[i].bDisplayed, m_pPictureStore[i].bInUse);
    if(m_pPictureStore[i].bRefPicture) counter++;
  }
  return counter;
}
