...
{
  struct TlibavcodecExt* tlib = (struct TlibavcodecExt *)c->opaque;

	int ret=tlib->ff_avcodec_default_get_buffer(c,pic);
	if (ret==0)
		tlib->OnGetBuffer(pic);
	return ret;
}
int TlibavcodecExt::reget_buffer(AVCodecContext *c, AVFrame *pic)
{
  struct TlibavcodecExt* tlib = (struct TlibavcodecExt *)c->opaque;
	...
	int ret=tlib->ff_avcodec_default_reget_buffer(c,pic);
	if (ret==0)
		tlib->OnRegetBuffer(pic);
	return ret;
}
void TlibavcodecExt::release_buffer(AVCodecContext *c, AVFrame *pic)
{
  struct TlibavcodecExt* tlib = (struct TlibavcodecExt *)c->opaque;
	...
	tlib->ff_avcodec_default_release_buffer(c,pic);
	tlib->OnReleaseBuffer(pic);
}
