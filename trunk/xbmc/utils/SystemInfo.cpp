...
bool CSysInfo::IsAppleTV()
{
  ...
  if (hw_model.find("appletv") != CStdString::npos)
    return true;
#endif
  return false;
}
...
bool CSysInfo::IsAeroDisabled()
{
  ...
  if (GetVersionEx((OSVERSIONINFO *)&osvi))
  {
    ...
    // at the moment - use this method only to detect if this is xp
    return false;
...
{
  CStdString result;
  result = "BOXEE/" + g_infoManager.GetLabel(SYSTEM_BUILD_VERSION) + " (";
#if defined(_WIN32)
  result += "Windows; ";
