...
public:
  ...
  XBMC_FORCE_INLINE bool PtInRect(const CPoint &point) const
  {
  ...
  XBMC_FORCE_INLINE const CRect &operator -=(const CPoint &point) 
  {
    x1 -= point.x;
    y1 -= point.y;
    x2 -= point.x;
  ...
  XBMC_FORCE_INLINE bool IsEmpty() const 
  {
#define EPSILON 10e-7
    return ((x2 - x1) * (y2 - y1)) < EPSILON;
  };

  XBMC_FORCE_INLINE float Width() const 
  {
    return x2 - x1;
  };

  XBMC_FORCE_INLINE float Height() const 
  {
    return y2 - y1;
  };
